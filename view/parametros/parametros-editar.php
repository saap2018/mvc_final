...
$Recordset6 = mysqli_query($link, "SELECT * FROM disponibilidad") or die(mysqli_error($link));
...
<form id="frm-Parametros" action="?c=Parametros&a=Guardar" method="post" enctype="multipart/form-data">
...
<div class="form-group">
  ...
  if($rows > 0) {
	  ...
	  for($i = 1; $i <= $limite;$i++)
			{
				$lugar = "UPDATE disponibilidad SET Disponibilidad ='Activo' where IdLugar ='$i'";
				$lugarparqueaderos= mysqli_query($link, $lugar);	
				//echo $cambiar;
			}
